...
- 💾 **智能备份** - 自动备份同步前的文件状态，支持版本还原
- 🔄 **Git-like 还原** - 交互式还原界面，支持 show、diff 等命令
...
DocSync 会自动备份每次同步前的文件状态，备份文件使用时间戳命名（如 `note.md.bak.20250107_164401`），同一批次同步的文件使用相同的时间戳。
...
# 还原整个文件夹的备份
python3 main.py --restore /path/to/folder
```

在还原界面中，你可以使用以下命令：
- `<数字>` - 还原指定序号的版本
- `show <数字>` - 显示指定版本的详细信息
- `diff <数字>` - 查看备份版本与当前文件的差异（类似 git diff）
- `log` - 重新显示版本列表
- `help` - 显示帮助信息
- `q/quit` - 退出
